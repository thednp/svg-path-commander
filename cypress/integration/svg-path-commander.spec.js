...
/** @typedef {import("../../types/index")} SVGPath */
...
describe('SVGPathCommander Class Test', () => {
  ...
  it('Test toCurve', () => {
    ...
    cy.get('@svgpath').then((svg) => {
        ...
        expect(svg[0].getAttribute('d')).to.equal('M2 0C0.8954305003384135 2.0290612532945332e-16 -1.3527075021963556e-16 0.8954305003384133 0 2C0 2 0 9.875 0 14C1.3527075021963556e-16 15.104569499661586 0.8954305003384133 16 2 16C8 16 10.25 16 14 16C15.104569499661586 16 16 15.104569499661586 16 14C16 8 16 5.75 16 2C16 0.8954305003384135 15.104569499661586 6.763537510981778e-17 14 0C8 0 5.75 0 2 0C2 0 2 0 2 0')
      })
  });
...
describe('SVGPathCommander Static Methods', () => {
    ...
    it(`Convert <${SHAPE}> to path`, () => {
      ...
      if (SHAPE === 'wombat') {
        cy.get('@svg').find(SHAPE).then((shape) => {
          try {
            SVGPathCommander.shapeToPath(shape[0], true)
          } catch (er) {
            expect(er).to.be.instanceOf(TypeError);
  ...
  shapeObjects.forEach((SHAPE) => {
    it(`Convert "${SHAPE.type}" Object to path`, () => {
      cy.get('@svg').should('exist').invoke('append', SVGPathCommander.shapeToPath(SHAPE))
  ...
  it(`Can do clonePath to pathArray as well as pathSegment`, () => {
    const pathArray = [["M",0,0],["L",182,72]];
    cy.log(`Can clone pathArray`).then(() => {
      expect(pathArray).to.deep.equal(SVGPathCommander.clonePath(pathArray));
    });
    cy.log(`Can clone pathSegment`).then(() => {
      expect(pathArray[0]).to.deep.equal(SVGPathCommander.clonePath(pathArray[0]));
    });
  });

  it(`Can do fixPath for closed and un-closed pathArray`, () => {
    const closed = [["M",0,0],["L",182,0],["L",91,72],["L",0,0],["Z"]];
    const closed1 = [["M",0,0],["L",182,0],["L",91,72],["Z"]];
    const unclosed = [["M",0,0],["L",182,0],["L",91,72],["L",0,0]];
    cy.log(`Can fix closed`).then(() => {
      expect(SVGPathCommander.fixPath(closed)).to.deep.equal(unclosed);
    });
    cy.log(`Can keep closed`).then(() => {
      expect(SVGPathCommander.fixPath(closed1)).to.deep.equal(closed1);
    });
    cy.log(`Can keep unclosed`).then(() => {
      expect(SVGPathCommander.fixPath(unclosed)).to.deep.equal(unclosed);
    });
  });
  ...
  it(`Can do reverseCurve`, () => {
    ...
    expect(SVGPathCommander.reverseCurve(pathReversed.segments)).to.deep.equal(path.segments);
  });
